...
mod tests {
    ...
    fn test_write_batch_1() {
        ...
        // 写数据之后未提交
        let put_res1 = wb.put(util::rand_kv::get_test_key(1), util::rand_kv::get_test_value(10));
        assert!(put_res1.is_ok());
        let put_res2 = wb.put(util::rand_kv::get_test_key(2), util::rand_kv::get_test_value(10));
    ...
    fn test_write_batch_2() {
        let mut opts = Options::default();
        opts.dir_path = PathBuf::from("/tmp/bitcask-rs-batch-2.0");
        opts.data_file_size = 64 * 1024 * 1024;
        let engine = Engine::open(opts.clone()).expect("failed to open engine");
        ...
        let put_res3 = wb.put(util::rand_kv::get_test_key(1), util::rand_kv::get_test_value(10));
